...
from linxo.client import Client, ENDPOINTS
...
class testClient(unittest.TestCase):
    ...
    def test_init(self):
        # nominal
        api = Client(ENDPOINT, CLIENT_ID, CLIENT_SECRET, ACCESS_TOKEN, REFRESH_TOKEN)
        self.assertEqual(CLIENT_ID, api._client_id)
        self.assertEqual(CLIENT_SECRET, api._client_secret)
        self.assertEqual(ACCESS_TOKEN, api._access_token)
        ...
        api = Client(ENDPOINT, CLIENT_ID, CLIENT_SECRET,
                     ACCESS_TOKEN, REFRESH_TOKEN, timeout=timeout)
        self.assertEqual(timeout, api._timeout)
    ...
    def test_init_from_custom_config(self):
        ...
        self.assertEqual('custom access_token', api._access_token)
    ...
    def test_generate_auth_url(self, m_req):
        api = Client(ENDPOINT, CLIENT_ID, CLIENT_SECRET, ACCESS_TOKEN)
    ...
    def test_generate_token(self, m_req, m_code):
        ...
        config.config.set('prod', 'access_token', ACCESS_TOKEN)
        ...
        m_req.assert_called_once_with(AUTH_URL + '/tokens',
                client_secret=CLIENT_SECRET,
                code='fake_code')

        config.read('/tmp/test')
        self.assertEqual(config.config.get('prod', 'access_token'), 'fake generated access_token')
